      ...
      <div className="w-full px-4 md:px-8 lg:px-12 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
            ...
            <span className="text-emerald-500 text-[10px] font-black tracking-wider uppercase">
              v1.2.0
            </span>
          </div>
        </div>
        ...
        {/* --- Desktop Links --- */}
        <div className="hidden xl:flex items-center bg-slate-200/50 dark:bg-white/5 px-1.5 py-1 rounded-2xl border border-slate-300/40 dark:border-white/5 backdrop-blur-md shadow-inner">
          {navLinks.map((link) => {
            const isActive =
              activeSection === link.id || link.href === currentPath;
            return (
              <a
                key={link.name}
                href={link.href}
                ...
                className={`relative px-4 py-2 text-[13px] font-bold transition-all duration-300 flex items-center gap-1.5 ${
                  isActive
              ...
              >
                {(hoveredPath === link.name || isActive) && (
                  <motion.div
                    ...
                    className="absolute inset-0 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.12)] dark:shadow-none"
                  />
                )}
